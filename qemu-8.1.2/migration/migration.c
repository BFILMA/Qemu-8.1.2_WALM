...
/* Added by Ilma */
bool max_iteration = false;
bool hb_enabled = false; /*Ilma: HB migration */
bool hb_switchpoint = false; /* Ilma: HB static switch point*/
static int hb_counter = 0; /* Ilma: HB iteration counter to monitor dirty pages */
uint64_t hb_diff = 60000000; /* Ilma: Difference in pending pages to determind hb switchpoint */
...
/* Added by Samindu */
static void populate_iteration_info(MigrationInfo *info, MigrationState *s)
{
	info->has_iterations = true;
...
static void fill_source_migration_info(MigrationInfo *info)
{
	...
	switch (state) {
		...
		case MIGRATION_STATUS_COMPLETED:
			...
			populate_ram_info(info, s);
			/* Added by Samindu */
			populate_iteration_info(info, s);
			/* --------------------------- */
			populate_vfio_info(info);
			break;
...
fail:
	...
	static MigIterateState migration_iteration_run(MigrationState *s)
	{
		...
		if (must_precopy <= s->threshold_size) {
			...
			printf("Pending = %ld | PreCopiable = %ld\n", pending_size, must_precopy);
		}

#ifdef ENABLE_DYNAMIC_BANDWIDTH
...
#else
		if (((((uint64_t)(pending_size/4096LLU)) <= 5000LLU) || max_iteration)) {
			trace_migration_thread_low_pending(pending_size);
			migration_completion(s);
			return MIG_ITERATE_BREAK;
			printf("Inside IFDEF");
		...
		if(hb_enabled){
			...
			printf("Dirty page difference %d\n", absValue);
			if (pending_size >= prev_pending_size || absValue < hb_diff){
				hb_counter++;
				printf("HB counter %d\n", hb_counter);
				if(hb_counter == 3){
					printf("--------------HB Switchpoint reached----------------\n");
					if (postcopy_start(s, &local_err))
					{
						migrate_set_error(s, local_err);
						error_report_err(local_err);
	...
	static void *migration_thread(void *opaque)
	{
		...
		while (migration_is_active(s)) {
			...
			if (iteration > 50) {
				printf("Maximum # of Iterations Reached\n");
				max_iteration = true;
			}
